...
import { useVaultDiff } from './lib/profile-api';
import type { VaultFile } from './lib/profile-api';
...
export function GraphifyPage({ profileId, profileName }: { profileId: string; profileName: string }) {
  ...
  return <div className="page graphify-page">
    ...
    {!state ? <section className="graphify-loading">Reading profile vault configuration...</section> : <>
      ...
      {state.vault_available && <VaultDiffSection profileId={profileId} />}
    </>}
  </div>;
}

function VaultDiffSection({ profileId }: { profileId: string }) {
  const { files, loading, since, reload } = useVaultDiff(profileId);
  const fmtSize = (bytes: number) => bytes < 1024 ? `${bytes}B` : `${(bytes / 1024).toFixed(1)}KB`;
  return <section className="vault-diff-section">
    <header>
      <div><h2>Vault changes</h2><p>{since ? `Since ${since.slice(0, 16).replace('T', ' ')}` : 'All vault files'}</p></div>
      <button className="button button-quiet" onClick={reload} disabled={loading}><RefreshCw size={13} className={loading ? 'spin' : ''} /> {loading ? 'Scanning...' : since ? 'Changes since last check' : 'Refresh'}</button>
    </header>
    {files.length === 0 ? <p className="vault-diff-empty">{loading ? 'Scanning vault...' : since ? 'No files changed since last check.' : 'No vault files found.'}</p> : <div className="vault-diff-list">{files.slice(0, 40).map((f: VaultFile) => <div key={f.path} className="vault-diff-row"><code className="vault-path">{f.path}</code><span className="vault-meta">{fmtSize(f.size)} · {f.modified_at.slice(0, 16).replace('T', ' ')}</span></div>)}{files.length > 40 && <p className="vault-diff-more">... and {files.length - 40} more</p>}</div>}
  </section>;
}
