...
function Centre({ onExit, mobileCompanion = false }: { onExit: () => void; mobileCompanion?: boolean }) {
  ...
  const [tasks, setTasks] = useStored<Task[]>('hermes.tasks', []);
  const [roles, setRoles] = useStored<Role[]>('hermes.roles', []);
  const [sources, setSources] = useStored<Source[]>('hermes.sources', []);
  ...
  const createRole = (role: Role) => { setRoles(current => [...current, role]); if (hermes.status.status === 'online' && !role.role.startsWith('Group chat')) void hermes.run(`A new agent has just been created in the Jarvis organisation. Agent name: ${role.name}. Agent responsibility: ${role.role}. Using only tools and providers that are actually configured, provision a dedicated email identity and a dedicated phone number for this agent. Check provider availability first. Ask for approval before any external, paid, irreversible, or administrator action. Never invent an address, number, provider result, credential, or completion. If provisioning is unavailable, report exactly what is missing and leave the agent identity state as Not configured. Never print passwords, API keys, recovery codes, or other secrets. Return a concise status with completed, blocked, and awaiting-approval items.`, `jarvis-${role.id}`).catch(() => undefined); };
